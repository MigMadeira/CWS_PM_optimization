...
class Parameters:
    def __init__(self, nphi, ntheta, ncoils, order, LENGTH_WEIGHT, LENGTH_THRESHOLD, CC_THRESHOLD, 
...
OUT_DIR = "./W7-X/coil_output/N=4_k=2/"                
os.makedirs(OUT_DIR, exist_ok=True)

# Read in the plasma equilibrium file
wout_name = './inputs/equilibria/wout_W7-X_standard_configuration.nc'
...
cws.change_resolution(3,3)
cws.extend_via_normal(0.52)

cws_full = SurfaceRZFourier.from_wout(surface_filename, range="full torus", nphi=nphi, ntheta=ntheta)
cws_full.change_resolution(3,3)
cws_full.extend_via_normal(0.52)


ncoils = 4
# Threshold and weight for the maximum length of each individual coil:
LENGTH_THRESHOLD = 43.449999999999996/4 + 2
LENGTH_THRESHOLD2 = 43.449999999999996
LENGTH_WEIGHT = 0
...
def create_cws_from_dofs(dofs_cws=cws.x, dofs_coils=None):
    ...
    Jcs = [LpCurveCurvature(c, 8, CURVATURE_THRESHOLD) for c in base_curves]
    Jmscs = [MeanSquaredCurvature(c) for c in base_curves]
    Jals = [ArclengthVariation(c) for c in base_curves]
    JF = (
        Jf
        + LENGTH_WEIGHT * sum(Jls)
        + LENGTH_CON_WEIGHT * sum([QuadraticPenalty(Jls[i], LENGTH_THRESHOLD, f="max") for i in range(len(base_curves))])
        + LENGTH_WEIGHT2 * QuadraticPenalty(sum(Jls), LENGTH_THRESHOLD2)
        + CC_WEIGHT * Jccdist
...
param = Parameters(nphi, ntheta, ncoils, order, LENGTH_WEIGHT, LENGTH_THRESHOLD, CC_THRESHOLD, 
                 CC_WEIGHT, CURVATURE_THRESHOLD, CURVATURE_WEIGHT, 
                 MSC_THRESHOLD, MSC_WEIGHT,ARCLENGTH_WEIGHT, tolerance)
