...
def cws_and_curves(factor):
    ...
    cws = SurfaceRZFourier.from_wout(wout, range="half period", ntheta=ntheta, nphi=nphi)
    cws_full = SurfaceRZFourier.from_wout(wout, range="full torus", ntheta=ntheta, nphi=int(nphi*2*s.nfp))
