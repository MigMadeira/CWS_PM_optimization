...
def cws_and_curves(R1, factor):
    # CREATE COIL WINDING SURFACE SURFACE
    quadpoints_phi = np.linspace(0, 1/(2*s.nfp), nphi, endpoint=True)
    quadpoints_theta = np.linspace(0, 1, ntheta, endpoint=True)

    cws = SurfaceRZFourier(quadpoints_phi=quadpoints_phi, quadpoints_theta=quadpoints_theta, nfp = s.nfp)
    cws.set_rc( 0, 0, s.get_rc(0,0))
    cws.set_rc( 1, 0, factor + R1)  #Minor plasma radius	0.53 metre
    cws.set_zs( 1, 0, factor + R1)  

    quadpoints_phi_full = np.linspace(0, 1, (2*s.nfp)*nphi, endpoint=True)
    quadpoints_theta_full = np.linspace(0, 1, ntheta, endpoint=True)
    cws_full = SurfaceRZFourier(quadpoints_phi=quadpoints_phi_full, quadpoints_theta=quadpoints_theta_full, nfp = s.nfp)
    cws_full.set_rc( 0, 0, s.get_rc(0,0))
    cws_full.set_rc( 1, 0, factor + R1)  
    cws_full.set_zs( 1, 0, factor + R1)  
...
factor_values = np.arange(0.62, 0.68, 0.02)

J_values = []

for i in factor_values:
    OUT_DIR2 = f"./W7-X/N=4_circular/coil_data/{i:.5f}/"
    os.makedirs(OUT_DIR2, exist_ok=True)
    cws, cws_full, base_curves, base_currents = cws_and_curves(0.53,i)
...
plt.xlabel("Added radius (m)")
plt.ylabel(r"$\Sigma B\cdot n$")

plt.savefig(f"{OUT_DIR}opt_CWS_scan.png")
#plt.show()
